...
export class editServerModal {
  ...
  constructor(
    ...
    tracking.trackFeature('cloud.server.edit');
  }
  ...
  public updateServer() {
    ...
    this.serverApiProvider.changeServerName(this.server.id, this.server.name).then(() => {
      this.serverApiProvider
        .changeProtection(this.server.id, this.server.protection.delete, this.server.protection.delete)
        .then(() => {
          loader.dismiss();
          this.dismiss();
        });
