...
export class consoleModal {
  ...
  constructor(
    ...
    tracking.trackFeature('cloud.server.console');
    this.serverApiProvider.requestConsole(this.server.id).then(response => {
      this.rfb = new RFB(document.getElementById('console_container'), response['wss_url'], {
      ...
      document.getElementById('noVNC_keyboardinput').addEventListener('input', event => {
        this.keyInput(event);
      });
      document.getElementById('noVNC_keyboardinput').addEventListener('focus', event => {
        this.onfocusVirtualKeyboard(event);
      });
      document.getElementById('noVNC_keyboardinput').addEventListener('blur', event => {
