...
describe('[3][1] Images List', () => {
  ...
  it('[1] first of all it should see three segment buttons', () => {
    ...
    expect(element(by.css('[data-test~="button_1"]')).getText()).toContain(page.getLocal('PAGE.IMAGES.TYPES.SNAPSHOT.TITLE').toUpperCase());

    // Backups
    expect(element(by.css('[data-test~="button_2"]')).isDisplayed()).toBeTruthy();
    expect(element(by.css('[data-test~="button_2"]')).getText()).toContain(page.getLocal('PAGE.IMAGES.TYPES.BACKUP.TITLE').toUpperCase());


    // Hetzner Images
    expect(element(by.css('[data-test~="button_3"]')).isDisplayed()).toBeTruthy();
    expect(element(by.css('[data-test~="button_3"]')).getText()).toContain(page.getLocal('PAGE.IMAGES.TYPES.SYSTEM.TITLE').toUpperCase());
