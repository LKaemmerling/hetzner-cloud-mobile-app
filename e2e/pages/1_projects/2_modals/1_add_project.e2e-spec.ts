...
describe('[1][2][1] Create Project', () => {
    ...
    it('[1] when i click the button it should open the modal and if you submit it there should be the error', () => {
        ...
        expect(
            element(
                by.xpath(
                    '/html[1]/body[1]/ion-app[1]/ion-modal[1]/div[1]/modal-addproject[1]/ion-header[1]/ion-toolbar[1]/div[2]/ion-title[1]/div[1]'
                )
            ).getText()
        ).toContain(page.getLocal('PAGE.PROJECTS.MODAL.ADD.TITLE'));
        element(by.buttonText(page.getLocal('ACTIONS.SAVE'))).click();
        browser.sleep(500);
        expect(
            element(
                by.xpath(
                    '/html[1]/body[1]/ion-app[1]/ion-modal[1]/div[1]/modal-addproject[1]/ion-content[1]/div[2]/p[1]'
                )
            ).getText()
        ).toContain(page.getLocal('PAGE.PROJECTS.MODAL.ADD.ERRORS.REQUIRED_NAME'));
    });
    it('[2] when i click the button it should open the modal and then i enter the correct credentials and the login-only menu entries should be there', () => {
        ...
        ).sendKeys('Hetzner Cloud App E2E');
        browser.sleep(500);
        element(
            by.xpath(
    ...
    it('[3] when i click the button is should open the modal and then i enter the wrong credentials', () => {
        ...
        ).sendKeys('Hetzner Cloud App E2E WRONG');
        browser.sleep(500);
        element(
            by.xpath(
                '/html[1]/body[1]/ion-app[1]/ion-modal[1]/div[1]/modal-addproject[1]/ion-content[1]/div[2]/ion-list[1]/ion-item[2]/div[1]/div[1]/ion-input[1]/input[1]'
            )
        ).sendKeys(browser.params.global.api_key + 'WRONG');
        browser.sleep(500);
        element(by.buttonText(page.getLocal('ACTIONS.SAVE'))).click();
        browser.sleep(1000);
        expect(
        ...
        ).toContain(page.getLocal('PAGE.PROJECTS.MODAL.ADD.ERRORS.INVALID_KEY'));
    });
    ...
    it('[4] a project name must be unique', () => {
        ...
        ).toContain(page.getLocal('PAGE.PROJECTS.MODAL.ADD.ERRORS.NAME_ALREADY_USED'));
    });
});
