...
describe('[5][2] Floating IP Actions', () => {
    ...
    it('[1] click on add button and create a floating ip', () => {
        ...
        expect(element(by.id('error_floating_ip')).getText()).toContain(
            page.getLocal('PAGE.FLOATING_IPS.MODAL.ADD.ERRORS.REQUIRED_DESCRIPTION')
        );
        ...
        expect(element(by.id('error_floating_ip')).isPresent()).toBeTruthy();
        expect(element(by.id('error_floating_ip')).getText()).toContain(
            page.getLocal('PAGE.FLOATING_IPS.MODAL.ADD.ERRORS.REQUIRED_NETWORK_PROTOCOL')
        );
        ...
        expect(element(by.id('error_floating_ip')).isPresent()).toBeTruthy();
        expect(element(by.id('error_floating_ip')).getText()).toContain(
            page.getLocal('PAGE.FLOATING_IPS.MODAL.ADD.ERRORS.REQUIRED_SERVER_OR_LOCATION')
        );
        ...
        expect(element(by.css("[data-test-floating-ip~='test-floating-ip'] h2")).getText()).toContain(
            'test-floating-ip'
        );
    });
    ...
    it('[3] click on floating ip action edit and edit the floating ip', () => {
        ...
        browser.sleep(1000);
        expect(element(by.css('[data-test-floating-ip~="test-floating-ip-R"] h2')).getText()).toContain(
            'test-floating-ip-R'
        );
