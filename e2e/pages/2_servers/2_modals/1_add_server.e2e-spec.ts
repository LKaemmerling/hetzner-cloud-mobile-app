...
describe('[2][2][1] Create Server', () => {
  ...
  it('[1] when i click the button it should open the modal then i should test the form and then i create the Server', () => {
    ...
    browser.sleep(500);
    expect(element(by.css('[data-test~="title"]')).getText()).toContain(page.getLocal('PAGE.SERVERS.MODAL.ADD.TITLE'));
    element(by.buttonText(page.getLocal('ACTIONS.ORDER'))).click();
    browser.sleep(500);
    expect(element(by.id('error_box')).getText()).toContain(page.getLocal('PAGE.SERVERS.MODAL.ADD.ERRORS.REQUIRED_TYPE'));
    browser.sleep(500);
    element(by.css('[data-test-type~="cx11"]')).click();
    browser.sleep(500);
    element(by.buttonText(page.getLocal('ACTIONS.ORDER'))).click();
    browser.sleep(500);
    expect(element(by.id('error_box')).getText()).toContain(page.getLocal('PAGE.SERVERS.MODAL.ADD.ERRORS.REQUIRED_LOCATION'));
    element(by.css('[data-test-datacenter~="Falkenstein_dc8"]')).click();
    browser.sleep(500);
    element(by.buttonText(page.getLocal('ACTIONS.ORDER'))).click();
    browser.sleep(500);
    expect(element(by.id('error_box')).getText()).toContain(page.getLocal('PAGE.SERVERS.MODAL.ADD.ERRORS.REQUIRED_IMAGE'));
    ...
    element(by.buttonText(page.getLocal('ACTIONS.ORDER'))).click();
    browser.sleep(500);
    expect(element(by.id('error_box')).getText()).toContain(page.getLocal('PAGE.SERVERS.MODAL.ADD.ERRORS.REQUIRED_NAME'));
